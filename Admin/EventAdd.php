...
<body>
    ...
    if (isset($_POST["btnSave"])) {
        ...
        if ($isValid) {
            // Get NGO ID
            $queryNGO = "SELECT NGOId FROM ngo WHERE Name = '$NGOName'";
    ...
    <form enctype="multipart/form-data" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <label for="ngo">Event NGO</label>
        <select id="ngo" name="ngo">
            <?php
            $queryNGO = "SELECT * FROM ngo";
            $resultNGO = mysqli_query($con, $queryNGO);
            while ($rowNGO = mysqli_fetch_array($resultNGO)) {
                echo "<option value='" . $rowNGO["Name"] . "'>" . $rowNGO["Name"] . "</option>";
            }
            ?>
        </select>
        <br />
